...
class HunterClient:
    ...
    def find_email(self, domain: str, first_name: str, last_name: str) -> dict:
        url = f'{self.API_BASE_URL}{self.API_EMAIL_FINDER_PATH}'
        query_params = {
            'domain': domain,
    ...
    def verify_email(self, email: str) -> dict:
        url = f'{self.API_BASE_URL}{self.API_EMAIL_VERIFIER_PATH}'
        query_params = {'email': email}
        return self._make_request(url, query_params)

    def _make_request(self, url: str, query_params: dict) -> dict:
        headers = {'Authorization': f'Bearer {self.api_key}'}
        response = requests.get(url, headers=headers, params=query_params, timeout=self.API_TIMEOUT)
